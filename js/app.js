...
(function () {
  ...
  function migrar(p) {
    if (p.config === 'F+N+T' || p.config === '2F+T') p.config = '2F+N+T';
    if (Number(p.tensao) === 127) p.tensao = 220;
  ...
  function renderForm() {
    ...
    <details data-sec="2"><summary>2 · Entrada de energia do cliente</summary><div class="sec-body">
      <div class="grid2">
        ${campo('Tensão / configuração', `<select data-chave="par-rede">${[['220|2F+N+T', '220 V · 2F+N+T'], ['220|3F+N+T', '220 V · 3F+N+T'], ['380|3F+N+T', '380 V · 3F+N+T']].map(([v, l]) => `<option value="${v}"${v === `${projeto.tensao}|${projeto.config}` ? ' selected' : ''}>${l}</option>`).join('')}</select>`)}
