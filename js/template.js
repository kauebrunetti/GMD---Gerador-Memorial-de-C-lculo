...
(function () {
  ...
  ${multi ? `<table style="width:100%;border-collapse:collapse;font-size:9.5px;margin-bottom:14px">
    <thead><tr style="background:${INK};color:#fff"><th style="${TH8}">Circuito</th><th style="${TH8}">Marca</th><th style="${TH8}">Modelo</th><th style="${TH8}">Tensão</th><th style="${TH8}">Corrente</th><th style="${TH8}">Potência</th><th style="${TH8}">FP</th><th style="${TH8}">Disjuntor</th><th style="${TH8}">Conector</th><th style="${TH8}">Saída</th></tr></thead>
    <tbody>${pts.map((x, i) => `<tr${i % 2 ? ` style="background:${BG2}"` : ''}><td style="${TD8};font-weight:600;color:${INK}">${x.id}</td><td style="${TD8};color:${TX1}">${ph(x.marca, '[XX]')}</td><td style="${TD8};color:${TX1}">${ph(x.modelo, '[XX]')}</td><td style="${TD8};color:${TX1}">${x.Ib ? x.V + ' V · ' + x.lig.rotulo.split('·').slice(1).join('·').trim() : `<span style="color:${G2}">[XX]</span>`}</td><td style="${TD8};color:${TX1}">${x.Ib ? fmt(x.Ib, 1) + ' A' : `<span style="color:${G2}">[XX]</span>`}</td><td style="${TD8};color:${TX1}">${phn(x.P, 'kW')}</td><td style="${TD8};color:${TX1}">${fmt(x.fp, 2)}</td><td style="${TD8};color:${TX1}">${x.disjuntor ? fmt(x.disjuntor) + ' A · C · ' + x.lig.polos : `<span style="color:${G2}">[XX]</span>`}</td><td style="${TD8};color:${TX1}">${x.conector ? esc(x.conector) : `<span style="color:${G2}">[XX]</span>`}</td><td style="${TD8};color:${TX1}">${x.P ? x.tipo : `<span style="color:${G2}">[XX]</span>`}</td></tr>`).join('\n    ')}</tbody>
  </table>`
  : `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:${LN1};${R};overflow:hidden;margin-bottom:16px">
    ${card31('Marca e modelo', `${ph(pt.marca, '[XX]')} · ${ph(pt.modelo, '[XX]')}`, 'equipamento previsto no projeto')}
    ${card31('Potência nominal', phn(pt.P, 'kW'), `fator de potência ${fmt(pt.fp, 2)}`)}
    ${card31('Alimentação', `${pt.V} V · ${phn(pt.Ib, 'A')}`, `${pt.lig.rotulo.split('·')[1] ? pt.lig.rotulo.split('·')[1].trim() : pt.lig.rotulo} · 60 Hz`)}
